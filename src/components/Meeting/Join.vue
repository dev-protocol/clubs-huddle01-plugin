...
const startTimeValid = computed(() => props.startTime && !isNaN(new Date(props.startTime).getTime()));
const expiryTimeValid = computed(() => props.expiryTime && !isNaN(new Date(props.expiryTime).getTime()));

const meetingHasStarted = computed(() => startTimeValid.value && !isFutureTimestamp(props.startTime as string));
const meetingIsActive = computed(() => startTimeValid.value && (!expiryTimeValid.value || isFutureTimestamp(props.expiryTime as string)));
...
<template>
	<button
		v-if="(startTime === undefined || !isFutureTimestamp(startTime)) && (expiryTime === undefined || isFutureTimestamp(expiryTime))"
